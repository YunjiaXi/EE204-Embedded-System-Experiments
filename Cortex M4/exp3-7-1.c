...
void UARTNonBlockingStringPut(const char *cMessage)
{
	while(*cMessage!='\0')
...
int AT_Match(char* buffer, int32_t* size)
{
	int i;
	...
	else if (strcmp(buffer, "@STUDENTCODE") == 0)
		return 2;
	else 
		return 0;
}

void AT_Instruction(char* buffer, int32_t* size)
{
	if (buffer[*size - 1] == '\n' && buffer[*size - 2] == '\r')
	{
		buffer[*size - 2] = '\0';
		switch (AT_Match(buffer, size))
		{
			case 1:
				UARTNonBlockingStringPut("CLASS: F1603301\r\n");
				break;
			case 2:
				UARTStringPut("CODE: 516030910179\r\n");
				break;
		}
...
void UART0_Handler(void)
{
	...
	uart0_int_status = UARTIntStatus(UART0_BASE, true);  // Get the interrrup
	...
	{
		buffer[counter++] = UARTCharGetNonBlocking(UART0_BASE);
		if (counter >= 2)
			AT_Instruction(buffer, &counter);
	}
...
void S800_UART_Init(void)
{
	...
	//initilize interrupt
	IntEnable(INT_UART0);
	//RT - Receive Timeout interrupt
	UARTIntEnable(UART0_BASE, UART_INT_RX | UART_INT_RT | UART_INT_TX); //Enable UART0 RX,RT interrupt
	IntMasterEnable();
}
void S800_GPIO_Init(void)
{
